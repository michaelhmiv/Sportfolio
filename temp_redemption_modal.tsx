...
export function RedemptionModal({
  ...
  const { data: dashboardData, isLoading: isDashboardLoading } = useQuery<VestingData>({
  ...
  useEffect(() => {
    if (!dashboardData?.vesting) {
      // Don't reset to 0 while loading - keep existing value
      return;
    }
  ...
  const setPlayerShares = (playerId: string, newShares: number) => {
    const totalOthers = distributions.reduce(
      (sum, d) => (d.player.id === playerId ? sum : sum + d.shares),
      0,
    );
  ...
  const loadPreset = (preset: PresetWithPlayers) => {
    ...
    setDistributions(
      preset.players.map((player, index) => ({
        player,
        shares: sharesPerPlayer + (index === 0 ? remainder : 0),
          ...
          <div className="flex-1 min-h-0 overflow-hidden px-4 sm:px-0">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col h-full">
              <TabsList className="grid w-full grid-cols-3 shrink-0">
                ...
                <TabsTrigger value="allocate" data-testid="tab-allocate">
                  Allocate
                </TabsTrigger>
              ...
              <TabsContent
                value="directory"
                className="flex-1 min-h-0 mt-2 gap-2 overflow-hidden data-[state=active]:flex data-[state=active]:flex-col"
              >
                <div className="flex flex-col sm:flex-row gap-2 shrink-0">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <ArrowUpDown
                        className={cn("h-4 w-4", sortDirection === "asc" && "rotate-180")}
                      />
                    </Button>
                  </div>
                </div>
                ...
                <div className="text-xs text-muted-foreground px-1 shrink-0">
                  {playersLoading ? (
                    ...
                    <>
                      Showing {Math.min(visibleLimit, directoryPlayers.length)} of{" "}
                      {playersData?.total || directoryPlayers.length} players
                        ...
                        distributions.map((dist) => (
                          ...
                          >
                            ...
                            <Input
                              ...
                              value={dist.shares}
                              onChange={(e) =>
                                setPlayerShares(dist.player.id, parseInt(e.target.value) || 0)
                              }
              ...
              <TabsContent
                value="presets"
                className="flex-1 min-h-0 mt-2 overflow-hidden data-[state=active]:flex data-[state=active]:flex-col"
              >
                <div className="border rounded-md overflow-hidden flex-1 min-h-0">
          ...
          <DialogFooter className="shrink-0 p-4 pt-2 sm:p-0 sm:pt-4">
            ...
            >
              {redeemMutation.isPending
                ? "Vesting..."
                : `Vest ${totalAllocated.toLocaleString()} Shares`}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
