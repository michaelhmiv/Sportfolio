...
export function AgentComposer({
  ...
  return (
    <div
      className="relative rounded-panel border border-border/60 bg-surface-raised/40 px-3 py-2"
      data-testid="agent-composer"
    >
      {/* Slash command autocomplete menu */}
      {slashMatches.length > 0 && (
        <div className="absolute bottom-full left-0 right-0 z-50 mb-1 overflow-hidden rounded-panel border border-border/60 bg-[hsl(var(--overlay-surface))] shadow-xl">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    index === selectedIndex
                      ...
                      : "text-content hover:bg-surface-raised/40",
                  )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-content/40">{cmd.label}</span>
                    </div>
                    <p className="text-[11px] text-content/30 truncate">{cmd.description}</p>
                  </div>
                </button>
              );
            })}
      ...
      <div className="flex items-end gap-2">
        <Textarea
          ref={textareaRef}
          className="min-h-[36px] flex-1 resize-none rounded-panel border-0 bg-transparent px-1 py-1 text-[13px] leading-5 text-content shadow-none placeholder:text-content/30 focus-visible:ring-0 sm:min-h-[52px]"
          value={value}
          onChange={(event) => {
        ...
        />
        <Button
          className="h-9 w-9 shrink-0 rounded-panel bg-gradient-to-r from-brand to-brand p-0 text-brand-foreground hover:from-brand hover:to-brand"
          onClick={() => onSend()}
          disabled={disabled}
      ...
      <div className="mt-1.5 hidden items-center justify-between px-1 text-[10px] text-content/25 sm:flex">
