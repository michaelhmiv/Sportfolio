  ...
  useEffect(() => {
    ...
    const finalizeOnce = async (): Promise<{ terminal: boolean }> => {
      if (!receipt) {
        setState("pending");
        setMessage(
          "The return URL did not include a receipt identifier, so this checkout cannot be verified yet.",
        );
      ...
      try {
        ...
        if (response.status === 401) {
          setState("error");
          setMessage(
            "Your sign-in session was not available after checkout. Sign in again, then return to this receipt.",
          );
          return { terminal: true };
        }
        ...
        if (
          ...
          setState("error");
          setMessage(
            "The payment amount did not match the selected purchase. Contact support before attempting another payment.",
          );
          return { terminal: true };
        }
    ...
    const poll = async () => {
      ...
      if (elapsedMs > 60_000) {
        setMessage(
          "Reconciliation is taking longer than expected. Use Check again or contact support with the receipt identifier.",
        );
        return;
      }
  ...
  const statePresentation =
    state === "credited"
      ? {
          ...
          tone: "bg-market-positive-subtle text-market-positive",
        }
      : state === "pending"
        ? {
            icon: Clock3,
            title: "Payment pending",
            tone: "bg-status-warning-subtle text-status-warning",
          }
        : state === "error"
          ? {
              icon: AlertCircle,
    ...
    <StatusSurface>
      <Card variant="default" className="border-border-strong shadow-medium">
        <CardContent className="p-6 text-center sm:p-9">
          <div
            className={`mx-auto flex h-16 w-16 items-center justify-center rounded-circle ${statePresentation.tone}`}
          >
            <StateIcon
              className={`h-8 w-8 ${state === "processing" ? "animate-spin" : ""}`}
              aria-hidden="true"
            />
          </div>
          <p className="mt-5 font-mono text-[11px] font-semibold uppercase tracking-[0.12em] text-content-subtle">
            Checkout status
          </p>
          ...
          <Link
            href="/"
            className="mt-6 inline-flex text-sm font-medium text-content-muted hover:text-brand"
          >
            Return to dashboard
          </Link>
