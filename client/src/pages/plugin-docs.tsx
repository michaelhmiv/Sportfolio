    ...
    <SurfaceLayout kind="public">
      ...
      <EditorialSection title="Supported capabilities">
        <div className="grid gap-4 md:grid-cols-2">
          {capabilities.map((item) => (
            <div key={item} className="flex gap-3 rounded-panel border border-border-subtle bg-surface p-5">
      ...
      <EditorialSection title="Preview before execution" className="bg-surface">
        <div className="max-w-3xl space-y-4 leading-7 text-content-muted">
          <p>Market trades, scouting, share stacking, daily boosts, community boosts, and liquidity operations use Sportfolio's staged-action system. The connected app first creates a current preview and pending action bundle.</p>
          <p>Review the displayed player, quantity, virtual cost, expected balance or holdings effect, and warnings. The action is finalized only after explicit confirmation of the same pending bundle. You may cancel a pending action instead.</p>
          <p>Some lower-risk account changes are immediate. The connected app should perform those changes only when the request clearly authorizes the exact operation.</p>
        </div>
      </EditorialSection>
      ...
      <EditorialSection title="Authentication and data boundaries">
        <div className="grid gap-5 md:grid-cols-2">
          <div className="rounded-panel border border-border-subtle bg-surface p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-content-muted">Public documentation and supported public sports information may not require account access. Personalized information and all account writes require an approved OAuth connection.</p>
          </div>
          <div className="rounded-panel border border-border-subtle bg-surface p-5">
            ...
            <p className="mt-2 text-sm leading-6 text-content-muted">Do not paste passwords, sign-in links, provider keys, access tokens, refresh tokens, MFA codes, or one-time authentication codes into ordinary conversation.</p>
          </div>
        </div>
        <a className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-brand hover:underline" href="/oauth/connected-apps/">
      ...
      <EditorialSection title="Virtual-only values" className="bg-surface">
        <p className="max-w-3xl leading-7 text-content-muted">Sportfolio shares, balances, position values, gains, losses, trades, and payouts are virtual game units. They are not securities, money, wagers, prizes, or withdrawable assets. Sportfolio does not offer real-money betting, gambling, or cash-out functionality.</p>
      </EditorialSection>
    </SurfaceLayout>
  );
}
