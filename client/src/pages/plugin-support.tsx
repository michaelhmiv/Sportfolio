    ...
    <SurfaceLayout kind="public">
      ...
      <EditorialSection title="Connection workflow">
        <div className="grid gap-5 md:grid-cols-2">
          {sections.map(({ icon: Icon, title, items }) => (
            <section
              key={title}
              className="rounded-panel border border-border-subtle bg-surface p-5 sm:p-6"
            >
              ...
              <ol className="mt-4 space-y-3">
                {items.map((item, index) => (
                  <li
                    key={item}
                    className="grid grid-cols-[24px_minmax(0,1fr)] gap-2 text-sm leading-6 text-content-muted"
                  >
      ...
      <EditorialSection title="Security boundary" className="bg-surface">
        <div className="flex max-w-3xl gap-4 rounded-panel border border-status-warning/30 bg-status-warning-subtle p-5">
          <AlertTriangle
            className="mt-0.5 h-6 w-6 shrink-0 text-status-warning"
            aria-hidden="true"
          />
          <div>
            <h3 className="font-bold text-content-strong">
              Never paste authentication secrets into conversation.
            </h3>
            <p className="mt-2 leading-7 text-content-muted">
              Sportfolio does not require API keys, passwords, provider keys, access tokens, refresh
              tokens, MFA codes, or one-time sign-in codes in ChatGPT or Codex. Complete
              authentication only through the Sportfolio sign-in and consent pages.
            </p>
          </div>
        </div>
      </EditorialSection>
      ...
      <EditorialSection title="Scope and limitations">
        <ul className="max-w-3xl space-y-3 text-content-muted">
          <li>
            Supported public, authenticated read, and approved write capabilities are exposed
            through the connected app.
          </li>
          <li>
            Admin, internal, debug, raw database, mobile-store billing, credential-management, and
            private provider controls remain unavailable.
          </li>
          <li>
            Sportfolio cannot place real-money bets, create cash prizes, cash out virtual values, or
            provide securities transactions.
          </li>
          <li>
            Sports schedules, statistics, rosters, injuries, and game statuses may be delayed or
            corrected by upstream data sources.
          </li>
        </ul>
        <div className="mt-7 flex flex-wrap gap-3">
          <Button asChild>
            <a href="/oauth/connected-apps/">Manage connected applications</a>
          </Button>
          <Button asChild variant="outline">
            <a href="/plugin/">Read app documentation</a>
          </Button>
          <Button asChild variant="outline">
            <a href={`mailto:${SPORTFOLIO_SUPPORT_EMAIL}`}>Email support</a>
          </Button>
        </div>
      </EditorialSection>
    </SurfaceLayout>
