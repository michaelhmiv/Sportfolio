...
export default function ConnectedAppsPage() {
  ...
  const selectedName = selectedGrant?.client?.name || selectedGrant?.client?.client_name || "this application";

  return (
    <SurfaceLayout kind="status" showFooter={false}>
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <div className="max-w-2xl">
          <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.12em] text-brand">Account security</p>
          <h1 className="mt-2 text-3xl font-black tracking-tight text-content-strong">Connected applications</h1>
          <p className="mt-3 leading-7 text-content-muted">Review and revoke OAuth applications that can access your Sportfolio account.</p>
        </div>

        {error ? (
          <div className="mt-7 rounded-panel border border-destructive/30 bg-destructive-subtle p-4 text-sm text-destructive" role="alert">{error}</div>
        ) : null}
          ...
          {loading ? (
            ...
            <Card variant="empty">
              <CardContent className="px-6 py-14 text-center">
                ...
                <h2 className="mt-4 text-xl font-bold text-content-strong">No connected applications</h2>
                <p className="mx-auto mt-2 max-w-md leading-6 text-content-muted">No external OAuth application currently has access to this Sportfolio account.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-4">
              {grants.map((grant) => {
                ...
                return (
                  <Card key={grant.id || grant.client_id} variant="default" className="border-border-strong">
                    <CardContent className="p-5 sm:p-6">
                      <div className="flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
                        <div className="min-w-0">
                          ...
                          {grant.granted_at ? <p className="mt-3 text-sm text-content-subtle">Connected {new Date(grant.granted_at).toLocaleString()}</p> : null}
                          {grant.client?.client_uri ? (
                            <a href={grant.client.client_uri} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-sm font-medium text-brand hover:underline">
                              Application website <ExternalLink className="h-3.5 w-3.5" aria-hidden="true" />
                            </a>
                          ) : null}
                        </div>
                        <Button type="button" variant="destructive" disabled={!grant.id || revoking === grant.id} onClick={() => setSelectedGrant(grant)} className="gap-2 sm:shrink-0">
                          {revoking === grant.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldOff className="h-4 w-4" />}
                          Disconnect
                        </Button>
                      </div>
                    </CardContent>
      ...
      <AlertDialog open={selectedGrant !== null} onOpenChange={(open) => !open && setSelectedGrant(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Disconnect {selectedName}?</AlertDialogTitle>
            <AlertDialogDescription>
              This revokes refresh access and prevents new Sportfolio requests through the application. Existing short-lived access may remain valid until it expires.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
