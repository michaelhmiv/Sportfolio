...
export default function OAuthConsentPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (detailsError || !data) throw new Error(detailsError?.message || "The authorization request is invalid or expired.");
        ...
        if (!cancelled) setError(cause instanceof Error ? cause.message : "Unable to load this authorization request.");
      } finally {
  ...
  const scopes = (details?.scope || "openid").split(/\s+/).map((scope) => scope.trim()).filter(Boolean);
          ...
          <CardContent className="p-0">
            <header className="border-b border-border-subtle bg-surface-raised px-6 py-6 sm:px-8">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.12em] text-brand">Secure account connection</p>
                  <h1 className="mt-2 text-2xl font-bold tracking-tight text-content-strong">Connect {clientName}</h1>
                  <p className="mt-2 leading-6 text-content-muted">Review what the application can access before continuing.</p>
                </div>
              </div>
            </header>
            ...
            <div className="p-6 sm:p-8">
              {loading ? (
                ...
                <div className="rounded-panel border border-destructive/30 bg-destructive-subtle p-4 text-sm text-destructive" role="alert">{error}</div>
              ) : details ? (
                <div className="space-y-6">
                  ...
                  <section className="rounded-panel border border-border-subtle bg-surface-raised p-5">
                    ...
                    <ul className="mt-3 space-y-3 text-sm leading-6 text-content-muted">
                      <li>Read supported account information such as virtual holdings, balance, trades, scouts, boosts, watchlists, collections, milestones, schedules, liquidity, news, and profile data.</li>
                      <li>Perform supported account and gameplay actions you explicitly request, including staged virtual trades, scouting, share stacking, boosts, community boosts, and liquidity changes.</li>
                      <li>Use previews and explicit confirmation before finalizing staged gameplay actions.</li>
                    </ul>
                  </section>
                  ...
                  <div className="flex items-start gap-2 text-xs leading-5 text-content-subtle">
                    ...
                    <span>After approval, you will return to {safeOrigin(details.redirect_uri)}.</span>
                  </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Button type="button" variant="outline" disabled={submitting !== null} onClick={() => void decide("deny")} data-testid="oauth-consent-deny">
                      {submitting === "deny" ? <Loader2 className="h-4 w-4 animate-spin" /> : "Deny"}
                    </Button>
                    <Button type="button" disabled={submitting !== null} onClick={() => void decide("approve")} data-testid="oauth-consent-approve">
