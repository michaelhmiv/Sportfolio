...
export default function Power() {
    ...
    const { data: eligibleData, isLoading: loadingEligible, refetch: refetchEligible, error: eligibleError } = useQuery<{
    ...
    const createCommunityBoostMutation = useMutation({
        ...
        onSuccess: () => {
            toast({ title: "Community Boost activated!", description: "1 Premium Share redeemed. +1x multiplier applied." });
            refetchBoosts();
            refetchEligible();
            refetchCommunity();
        },
        onError: (error: Error) => {
            toast({ title: "Community Boost failed", description: error.message, variant: "destructive" });
        },
    });
    ...
    if (eligibleData?.eligiblePlayers) {
        console.log(`[Power] Total eligible: ${eligibleData.eligiblePlayers.length}, Filtered: ${filteredPlayers.length}, Search: "${search}"`);
        eligibleData.eligiblePlayers.forEach((ep, i) => {
            console.log(`[Power] ${i}: ${ep.player?.firstName} ${ep.player?.lastName} - qty: ${ep.totalShares}, pl: ${ep.powerLevel}`);
        });
    }

    const filteredCommunityBoosts = communityData?.communityBoosts?.filter(cb => {
                ...
                <ErrorBoundary>
                    ...
                    <div className="mb-3">
                        ...
                        <div className="flex items-center gap-2 mb-3">
                            <Button variant="ghost" size="icon" onClick={() => setSelectedDate(subDays(selectedDate, 1))} className="h-8 w-8">
                                <ChevronLeft className="w-4 h-4" />
                            </Button>
                            <div className="flex-1 text-center text-sm font-medium">{format(selectedDate, 'EEE, MMM d')}</div>
                            <Button variant="ghost" size="icon" onClick={() => setSelectedDate(addDays(selectedDate, 1))} className="h-8 w-8">
                        ...
                        <div className="flex items-center gap-2 text-xs flex-wrap">
                            <Badge variant="outline" className="gap-1">
                                <span className="w-2 h-2 rounded-full bg-green-500" />{activeBoosts}/4 active
                            </Badge>
                            {lockedBoosts > 0 && (
                                <Badge variant="outline" className="gap-1">
                                    <span className="w-2 h-2 rounded-full bg-yellow-500" />{lockedBoosts} live
                                </Badge>
                            )}
                            {userPremiumShares > 0 && (
                                <Badge className="gap-1 bg-amber-500/20 text-amber-500 border-amber-500/30">
                                    <Star className="w-3 h-3" />{userPremiumShares} Premium
                                </Badge>
                            )}
                        </div>
                    </div>
                    ...
                    <Card className="mb-3 bg-destructive/5 border-destructive/20">
                        <CardContent className="p-3">
                            <div className="flex items-start gap-2">
                                ...
                                <p className="text-xs text-muted-foreground">Shares are <span className="text-destructive font-medium">burned</span> when games start.</p>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                            {MULTIPLIER_SLOTS.map(({ tier, label, color }) => {
                                ...
                                return (
                                    ...
                                    >
                                        <div className={cn("absolute top-0 right-0 px-2 py-0.5 text-white text-xs font-bold", color)}>{label}</div>
                                        <CardContent className="p-2">
                                            {boost ? (
                                                <div className="space-y-2">
                                                    <div className="flex items-center gap-1 mb-1">
                                                        <Badge variant="outline" className="text-[10px] px-1 py-0 h-5">{boost.sport}</Badge>
                                                    </div>
                                                    <div className="flex items-start justify-between gap-1 min-w-0">
                                                        <div className="min-w-0">
                                                            <div className="font-medium text-sm truncate">
                                                                {boost.player && <PlayerName playerId={boost.player.id} firstName={boost.player.firstName} lastName={boost.player.lastName} className="text-sm" />}
                                                            </div>
                                                            <div className="flex items-center gap-1.5 text-xs">
                                                                <span className="text-muted-foreground">Power</span>
                                                                <span className="font-mono text-purple-400 font-medium">{boost.powerLevel}</span>
                                                            </div>
                                                            <div className="flex items-center gap-1.5 text-xs">
                                                                <span className="text-muted-foreground">Total</span>
                                                                <span className="font-mono font-bold text-primary">
                                                                    {(parseFloat(boost.powerLevel) * (tier + boost.communityBoostCount)).toFixed(2)}
                                                                </span>
                                                        ...
                                                        {boost.status === "active" && (
                                                            <Button size="icon" variant="ghost" onClick={(e) => { e.stopPropagation(); removeBoostMutation.mutate(boost.id); }} disabled={removeBoostMutation.isPending} className="h-6 w-6 shrink-0">
                                                                <X className="w-3 h-3" />
                                                            </Button>
                                                        )}
                                                    </div>
                        ...
                        <CardHeader className="pb-2">
                            <div className="flex items-center justify-between">
                                <CardTitle className="text-sm flex items-center gap-2">
                                    <Users className="w-4 h-4" />Community Boosts
                                </CardTitle>
                                <div className="flex items-center gap-1">
                                    <Button
                                        size="sm"
                                        ...
                                        <div key={cb.playerId} className="p-3 flex items-center justify-between gap-2">
                                            <div className="flex items-center gap-2 min-w-0 flex-1">
                                                <div className="w-8 h-8 rounded-full bg-amber-500/20 flex items-center justify-center shrink-0">
                                                    <span className="text-xs font-bold text-amber-500">{cb.player.firstName[0]}{cb.player.lastName[0]}</span>
                                                </div>
                                                <div className="min-w-0">
                                                    <div className="font-medium text-sm truncate">
                                                        <PlayerName playerId={cb.player.id} firstName={cb.player.firstName} lastName={cb.player.lastName} className="text-sm" />
                                                    </div>
                                                    <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                                        <span>{cb.player.team}</span>
                                                        <span>•</span>
                                                        <Badge variant="outline" className="text-[10px] px-1 py-0 h-4">{cb.sport}</Badge>
                                                        <span>•</span>
                    ...
                    <Card>
                        <CardHeader className="pb-2">
                            <div className="flex items-center justify-between">
                                <CardTitle className="text-sm flex items-center gap-2">
                                    <History className="w-4 h-4" />Payouts
                                </CardTitle>
                                ...
                                <div className="divide-y max-h-[300px] overflow-y-auto">
                                    {historyData.payouts.map((payout) => (
                                        <div key={payout.id} className="p-3 flex items-center justify-between gap-2">
                                            <div className="min-w-0">
                                                <div className="font-medium text-sm truncate">
                                                    {payout.player && <PlayerName playerId={payout.player.id} firstName={payout.player.firstName} lastName={payout.player.lastName} className="text-sm" />}
                                                </div>
                                                <div className="text-xs text-muted-foreground">{payout.sharesUsed} × {payout.fantasyPoints} FP × {payout.multiplier}x • {format(new Date(payout.createdAt), "MMM d")}</div>
                                            </div>
                                            <span className="font-mono font-bold text-green-500 shrink-0">+${payout.payoutAmount}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                    ...
                    <Dialog open={playerSelectorOpen} onOpenChange={(open) => {
                        ...
                        <DialogContent className="max-w-md max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Choose a player for {MULTIPLIER_SLOTS.find(s => s.tier === selectedSlot)?.label} slot
                                </DialogDescription>
                            </DialogHeader>
                            ...
                            <div className="flex-1 overflow-y-auto">
                                {loadingEligible ? (
                                    ...
                                    <div className="py-8 text-center text-destructive text-sm">
                                        ...
                                        <div className="text-xs font-mono bg-destructive/10 p-2 rounded mb-2 overflow-auto max-h-32">{eligibleError.message}</div>
                                        <Button size="sm" variant="outline" onClick={() => refetchEligible()} className="mt-2">
                                            Retry
                                        </Button>
                                    </div>
