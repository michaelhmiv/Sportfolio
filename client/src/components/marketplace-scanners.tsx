...
export function DashboardScanners() {
  ...
  const descriptions: Record<string, string> = {
    undervalued:
      "Players priced significantly below their calculated fair value based on recent performance.",
    ...
    return (
      <CarouselItem className={`${isExpanded ? "basis-[95%] lg:basis-[48%]" : "basis-[85%]"} pl-4`}>
        <div className={`terminal-shell h-full overflow-hidden ${color.border} flex flex-col`}>
          ...
          {/* Content List */}
          <div
            className={`p-2 flex-1 overflow-y-auto ${isExpanded ? "space-y-1" : "grid grid-cols-2 gap-2"}`}
          >
          ...
          <div className="p-2 border-t bg-muted/20 text-center">
            <Button
              ...
              onClick={() => {
                const url =
                  type === "risers"
                    ? "/pools?sortBy=change&sortOrder=desc"
                    : type === "pools"
                      ? "/pools?sortBy=tvl&sortOrder=desc"
                      : type === "sentiment"
                        ? "/pools?sortBy=sentiment&sortOrder=desc"
                        : type === "undervalued"
                          ? "/pools?sortBy=undervalued&sortOrder=asc"
                          : "/pools";
...
function DesktopScannerGrid({
  ...
  const topMc = [...scanData.undervalued, ...scanData.momentum]
    ...
    .sort(
      (a, b) =>
        parseFloat(b.player.marketCap?.toString() || "0") -
        parseFloat(a.player.marketCap?.toString() || "0"),
    )
    .slice(0, 6);
