...
function ScoutRoster({ playerId, globalTotal }: { playerId: string, globalTotal: number }) {
    ...
    if (isLoading) return <div className="p-4 text-center text-xs text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin mx-auto mb-1" />Loading scouts...</div>;
                    ...
                    <div key={idx} className="flex items-center justify-between text-xs p-1.5 rounded hover:bg-muted/50">
                        <div className="flex items-center gap-2">
                            ...
                            <Avatar className="h-5 w-5 border">
                                <AvatarFallback className="text-[9px]">{entry.user?.username?.[0] || "?"}</AvatarFallback>
                            </Avatar>
                            <span className="font-medium truncate max-w-[100px]">{entry.user?.username || "Unknown User"}</span>
                        </div>
                        <div className="flex items-center gap-2">
        ...
        <Popover>
            ...
            <PopoverContent className="w-80 p-4 text-xs space-y-3" align="end">
                ...
                <div className="space-y-2 text-muted-foreground">
                    <p>
                        <strong className="text-foreground">Goal:</strong> Earn free shares of players by scouting them.
                    </p>
                    <p>
                        <strong className="text-foreground">Mechanic:</strong> Every player mints <span className="text-green-600 font-bold">60 shares per hour</span>.
                    </p>
                    <p>
                        <strong className="text-foreground">Your Reward:</strong> You split these 60 shares with other scouts based on how many scouts you assign.
                    </p>
                    <div className="bg-muted/50 p-2 rounded border font-mono text-[10px] text-center">
                        (Your Scouts / Total Scouts) × 60 = Hourly Shares
                    </div>
                    <p>
                        <span className="text-amber-600 font-medium">Strategy:</span> Scout "hidden gems" with few other scouts to get a larger slice of the pie!
                    </p>
                </div>
            </PopoverContent>
        </Popover>
...
export function ScoutDashboardModal() {
    ...
    const { data: scoutStatus } = useQuery<{ earnedMinutes: number; nextDistribution: string; perPlayer?: Record<string, number> }>({
    ...
    const estimatedSharesPerHour = assignments.reduce((acc, curr) => {
        ...
        if (activeTab === 'scouts') {
            ...
            rawList = scoutedPlayers.filter(p => {
                ...
                if (searchQuery) {
                    ...
                    return p.firstName.toLowerCase().includes(q) || p.lastName.toLowerCase().includes(q) || p.team.toLowerCase().includes(q);
                }
                return true;
            });
            ...
            if (isLocalPortfolioView) {
                const heldPlayers = portfolioData?.holdings
                    .filter(h => h.player)
                    .map(h => ({
                        ...h.player,
                ...
                rawList.sort((a, b) => sortDirection === 'asc' ? a.sharesOwned - b.sharesOwned : b.sharesOwned - a.sharesOwned);
            } else {
                    ...
                    <div className="bg-card border rounded-md p-2 shadow-sm space-y-2">
                        <div className="flex justify-between items-center text-xs">
                            <span className="text-muted-foreground flex items-center gap-1.5">
                                <span className={cn("font-bold text-base", remaining === 0 ? "text-amber-600" : "text-foreground")}>
                                    {totalScouts}
                                </span>
                                <span className="text-muted-foreground">/ {maxScouts} Assigned</span>
                            </span>
                        </div>
                        <Progress value={(totalScouts / maxScouts) * 100} className="h-1.5" indicatorClassName={cn("bg-amber-500", remaining === 0 && "animate-pulse")} />
                ...
                <div className="flex-1 overflow-auto bg-card">
                    <div className="w-full">
                        <div className="px-4 py-2 bg-muted/20 border-b text-[10px] text-muted-foreground flex justify-between items-center">
                            ...
                            {activeTab === 'market' && sortField !== 'shares' && playersData?.total && playersData.total > limit && (
                                <span className="text-amber-600 font-medium animate-pulse">Scroll to load more</span>
                            )}
                        </div>
                        {/* Table Header */}
                                    ...
                                    <div key={player.id} className="group flex flex-col transition-colors border-b last:border-0">
                                        ...
                                        >
                                            ...
                                            <div className="w-8 flex justify-center">
                                                ...
                                                >
                                                    {expandedPlayerId === player.id ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                                                </Button>
                                            </div>
                                        ...
                                        {expandedPlayerId === player.id && (
                                            <div className="bg-muted/30 border-t p-2 pl-12 animate-in slide-in-from-top-2 duration-200">
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div className="space-y-2">
                                                        ...
                                                        <div className="bg-background border rounded p-2 text-xs space-y-1">
                                                            ...
                                                            <div className="flex justify-between">
                                                                ...
                                                                <span className="font-mono">{player.scoutCount} ({player.globalScoutCount > 0 ? ((player.scoutCount / player.globalScoutCount) * 100).toFixed(1) : 0}%)</span>
                                                            </div>
                                                    ...
                                                    <div className="space-y-2">
                                                        <ScoutRoster playerId={player.id} globalTotal={player.globalScoutCount} />
                                                    </div>
                                                </div>
                                            </div>
                                        )}
