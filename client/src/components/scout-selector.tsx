        ...
        <div className="border rounded-md p-2 bg-gradient-to-br from-amber-500/5 to-orange-500/5">
            <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                    ...
                    {scoutData?.isPremium && (
                        <Badge variant="secondary" className="h-4 px-1 text-[9px] bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-3">
                    ...
                    <div className="w-8 text-center">
                        <span className={`text-sm font-bold ${sliderValue > 0 ? 'text-amber-600' : 'text-muted-foreground'}`}>
                            {sliderValue}
                        </span>
                    </div>
                </div>
                ...
                <div className="flex justify-between items-center text-[10px] px-1 py-1.5 bg-background/50 rounded border border-border/50">
                    <div className="text-muted-foreground">
                        Global Scouts: <span className="font-mono text-foreground">{otherScouts + sliderValue}</span>
                    </div>
                    <div className="font-medium">
                        Est. Earnings: <span className="text-amber-600 font-bold">{projectedShare.toFixed(2)}</span> / hr
                    </div>
                </div>
                    ...
                    >
                        {assignMutation.isPending ? (
                            ...
                            <>
                                ...
                                {sliderValue === 0 ? "Remove Scouts" : `Assign ${sliderValue} Scout${sliderValue > 1 ? 's' : ''}`}
                            </>
                        )}
                    </Button>
                )}
