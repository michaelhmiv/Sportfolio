...
function usePresentation(): {
  ...
  updateWidgetState: (patch: Partial<WidgetState>, modelContext?: JsonRecord, contextText?: string) => void;
} {
  const [payload, setPayload] = useState<PresentationPayload | null>(() => normalize(getHostSnapshot().toolOutput));
  ...
  return (
    <div className="row">
      ...
      <div>
        ...
        <div className="sub">{[text(player.team), text(player.position), text(player.sport)].filter(Boolean).join(" · ")}</div>
      </div>
    </div>
  );
}
...
function Chart({ points }: { points: unknown[] }) {
  const values = points.map(asRecord).map((point) => num(point.price, Number.NaN)).filter(Number.isFinite);
  const line = useMemo(() => {
...
function PendingAction({ pending, onDone }: { pending: JsonRecord; onDone?: () => Promise<void> | void }) {
  ...
  const finalize = async (tool: "confirm_pending_action" | "cancel_pending_action") => {
    if (!transactionId) {
      setMessage("This staged action is missing its transaction id. Refresh and stage the action again.");
      return;
    }
    setBusy(true);
    try {
  ...
  return (
    <section className="section">
      ...
      <Notice><strong>{text(pending.summary, text(transaction.summary, "Sportfolio action ready for confirmation"))}</strong></Notice>
      {arr(pending.warnings ?? transaction.warnings).filter((item): item is string => typeof item === "string").map((warning) => <Notice key={warning}>{warning}</Notice>)}
      <div className="row section">
        <button className="btn primary" disabled={busy || !transactionId} onClick={() => void finalize("confirm_pending_action")}>Confirm</button>
        <button className="btn" disabled={busy || !transactionId} onClick={() => void finalize("cancel_pending_action")}>Cancel</button>
      </div>
...
function PlayerMarket({ payload, update, widgetState, updateWidgetState }: ViewProps) {
  ...
  const [amount, setAmount] = useState(widgetState.playerId === playerId ? widgetState.draftAmount || "" : "");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        if (sequence === quoteSequence.current) setQuote(Object.keys(asRecord(body.quote)).length ? asRecord(body.quote) : body);
      } catch (error) {
    ...
    <div className="panel">
      ...
      <div className="content">
        ...
        <div className="between" style={{ marginBottom: 10 }}>
          ...
          <div className="row">
            {!fullscreen ? <button className="btn primary" onClick={() => void requestDisplayMode("fullscreen")}>Open market</button> : null}
        ...
        <div className="context-actions section">
          <button className="btn" onClick={() => void sendFollowUpMessage(`Analyze ${text(player.displayName, "this player")}'s Sportfolio market and recent performance.`)}>Analyze this market</button>
        </div>
        {fullscreen && capabilities.canTrade === true ? (
          <section className="section">
            <div className="between"><h3>Trade this market</h3><div className="segments"><button aria-pressed={side === "buy"} onClick={() => chooseSide("buy")}>Buy</button><button aria-pressed={side === "sell"} onClick={() => chooseSide("sell")}>Sell</button></div></div>
            <div className="grid">
...
function Portfolio({ payload, update, updateWidgetState }: ViewProps) {
  ...
  return (
    <div className="panel">
      <header className="head"><div><div className="title">Your Sportfolio portfolio</div><div className="sub">Live virtual holdings and market exposure</div></div><button className="btn" onClick={() => void requestDisplayMode("fullscreen")}>Fullscreen</button></header>
      <div className="content">
        <div className="grid"><Stat label="Portfolio value" value={money(summary.totalValue)} /><Stat label="Available balance" value={money(summary.availableBalance)} /><Stat label="Cost basis" value={money(summary.costBasis)} /><Stat label="Holdings" value={quantity(summary.holdingCount, 0)} /></div>
        <div className="table-wrap section"><table className="table"><thead><tr><th>Player</th><th>Shares</th><th>Price</th><th>Value</th><th /></tr></thead><tbody>{holdings.map((holding) => { const player = asRecord(holding.player); const playerId = text(player.playerId); const name = text(player.displayName); return <tr key={playerId}><td><strong>{name}</strong><div className="muted">{[text(player.team), text(player.sport)].filter(Boolean).join(" · ")}</div></td><td>{quantity(holding.quantity)}</td><td>{money(holding.currentPrice)}</td><td>{money(holding.positionValue)}</td><td><button className="btn" onClick={() => void open(playerId, name)}>View market</button></td></tr>; })}</tbody></table></div>
...
function Movers({ payload, update, updateWidgetState }: ViewProps) {
  ...
  const open = async (playerId: string, playerName: string) => {
    updateWidgetState({ selectedPlayerId: playerId }, { selectedPlayerId: playerId }, `Sportfolio widget selection: ${playerName || playerId}.`);
    const next = normalize(await callTool("render_player_market", { playerId, range: "1D" }));
    if (next) update(next);
  };
  return (
    <div className="panel">
      <header className="head"><div><div className="title">Sportfolio market movers</div><div className="sub">{text(payload.data.category, "gainers").replaceAll("_", " ")}</div></div></header>
      <div className="content"><div className="carousel">{items.map((item) => { const player = asRecord(item.player); const playerId = text(player.playerId); const name = text(player.displayName); const change = num(item.changePercent); return <article className="card" key={playerId}><Identity player={player} /><div className="between section"><Stat label="Price" value={money(item.currentPrice)} /><strong className={change > 0 ? "positive" : change < 0 ? "negative" : "muted"}>{percent(change)}</strong></div><div className="muted">Volume {money(item.volume)}</div><button className="btn primary section" style={{ width: "100%" }} onClick={() => void open(playerId, name)}>View market</button></article>; })}</div>{items.length ? null : <Notice>No qualifying markets were found.</Notice>}</div>
      <footer className="footer">Updated {timestamp(payload.asOf)}.</footer>
    </div>
  );
}
...
function App() {
  ...
  return <><style>{CSS}</style><main className="shell">{payload ? <>{payload.warnings.map((warning) => <Notice key={warning}>{warning}</Notice>)}{payload.view === "player_market" ? <PlayerMarket {...presentation} payload={payload} /> : null}{payload.view === "portfolio" ? <Portfolio {...presentation} payload={payload} /> : null}{payload.view === "market_movers" ? <Movers {...presentation} payload={payload} /> : null}{payload.view === "liquidity" ? <Liquidity {...presentation} payload={payload} /> : null}{payload.view === "trade_preview" ? <div className="panel"><header className="head"><div><div className="title">Sportfolio action review</div><div className="sub">Confirm only the exact staged transaction</div></div></header><div className="content"><PendingAction pending={payload.data} /></div></div> : null}</> : <div className="panel loading">Loading Sportfolio…</div>}</main></>;
}
