...
Core scoring comes from the box score. **This is the simplified Sportfolio NHL scoring model, not a claim of full DraftKings NHL Classic equivalence.** Short-handed and shootout event enrichment is deliberately left observable as `scoringEnrichment.status: "not_attempted"`; those stats are never guessed. NHL stat records carry `settlementEligible: false`, so payout/boost settlement must refuse NHL until a bounded play-by-play enrichment path is added. A shutout additionally needs explicit final/starter evidence, not just zero goals against.
