...
Core scoring comes from the box score. Short-handed and shootout event enrichment is deliberately left observable as `scoringEnrichment.status: "not_attempted"` until a bounded play-by-play implementation is added; it is never guessed. A shutout additionally needs explicit final/starter evidence, not just zero goals against.
