...
export async function createNFLContests(): Promise<{ contestsCreated: number; errors: number }> {
  ...
  try {
    ...
    console.log(
      `[NFL Contest Creation] Looking for games between ${now.toISOString()} and ${future.toISOString()}`,
    );
    ...
    console.log(
      `[NFL Contest Creation] Finished: ${contestsCreated} created, ${errorCount} errors`,
    );
  } catch (error: any) {
