...
function isStoryFresh(headline: string, briefing: string): boolean {
  ...
  for (const indicator of staleIndicators) {
    if (combinedText.includes(indicator)) {
      console.log(
        `[News] Rejected stale content (contains "${indicator}"): "${headline.substring(0, 50)}..."`,
      );
      return false;
    }
  }
...
function validateUrlMatchesHeadline(headline: string, url: string | null): boolean {
  ...
  // Count matches
  const matches = headlineWords.filter(
    (word) =>
      ![
        "the",
        "and",
        ...
        "general",
        "manager",
      ].includes(word) && urlLower.includes(word),
  ).length;
    ...
    try {
      ...
      if (headlineMatch && briefingMatch) {
        ...
        if (sourceUrl && !validateUrlMatchesHeadline(headline, sourceUrl)) {
          console.warn(
            `[News] URL mismatch for: "${headline.substring(0, 50)}..." - URL: ${sourceUrl.substring(0, 60)}...`,
          );
...
export async function fetchNews(progressCallback?: ProgressCallback): Promise<NewsResult> {
  try {
    ...
    const deleted = await db.delete(newsFeed).where(lt(newsFeed.createdAt, sevenDaysAgo));
