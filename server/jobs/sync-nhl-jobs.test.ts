...
const { storage, nhlApi } = vi.hoisted(() => ({
  storage: {
    getPlayersBySport: vi.fn(), updatePlayer: vi.fn(), upsertPlayer: vi.fn(),
    getDailyGamesBySport: vi.fn(), getDailyGameByGameId: vi.fn(), getGameStatsByGameId: vi.fn(),
    createDailyGame: vi.fn(), updateDailyGame: vi.fn(), upsertPlayerGameStats: vi.fn(), getPlayersByIds: vi.fn(),
  },
  nhlApi: {
    getSeasons: vi.fn(), getStandings: vi.fn(), getRoster: vi.fn(), getScore: vi.fn(),
    getBoxscore: vi.fn(), getSchedule: vi.fn(),
  },
}));
...
const rosterPlayer = (id: number, positionCode = "C") => ({
  id, firstName: { default: "First" }, lastName: { default: String(id) }, positionCode,
});
const completeRoster = (seed: number, included: Array<ReturnType<typeof rosterPlayer>> = []) => ({
  forwards: [...included, ...Array.from({ length: 12 - included.length }, (_, index) => rosterPlayer(seed + index))],
...
const game = (id: number, state = "LIVE") => ({
  id, gameState: state, startTimeUTC: "2026-01-02T04:00:00Z",
  homeTeam: { abbrev: "BOS", score: 1 }, awayTeam: { abbrev: "NYR", score: 0 },
});
const finalBoxscore = (playerId = 77) => ({
  playerByGameStats: {
    homeTeam: { forwards: [{ playerId, name: { default: "Call Up" }, position: "C", goals: 1, assists: 0, points: 1 }], defense: [], goalies: [] },
...
describe("syncNhlRoster", () => {
  it("reconciles a transfer globally regardless of team iteration order", async () => {
    storage.getPlayersBySport.mockResolvedValue([
      { id: "nhl_9", team: "AAA", isActive: true }, { id: "nhl_10", team: "AAA", isActive: true },
    ]);
    nhlApi.getStandings.mockResolvedValue({ standings: [{ abbrev: "BBB" }, { abbrev: "AAA" }] });
    nhlApi.getRoster.mockImplementation((team: string) => Promise.resolve(
      team === "AAA" ? completeRoster(1000, [rosterPlayer(10)]) : completeRoster(2000, [rosterPlayer(9)]),
    ));
  ...
  it("rejects a one-player nonempty roster and never mass-deactivates the team", async () => {
    storage.getPlayersBySport.mockResolvedValue(Array.from({ length: 20 }, (_, index) => ({
      id: `nhl_${index + 1}`, team: "AAA", isActive: true,
    })));
    nhlApi.getStandings.mockResolvedValue({ standings: [{ abbrev: "AAA" }] });
    nhlApi.getRoster.mockResolvedValue({ forwards: [rosterPlayer(1)], defensemen: [], goalies: [] });
  ...
  it("makes both teams non-authoritative when a player appears on both rosters", async () => {
    ...
    nhlApi.getRoster.mockImplementation((team: string) => Promise.resolve(
      completeRoster(team === "AAA" ? 1000 : 2000, [rosterPlayer(9)]),
    ));

    const result = await syncNhlRoster();

    expect(result.successfulTeams).toBe(0);
    expect(result.errors.some((message) => message.includes("both teams retained non-authoritatively"))).toBe(true);
    expect(storage.updatePlayer).not.toHaveBeenCalled();
  });
});
...
describe("syncNhlStats", () => {
  ...
  it("skips a completed game on a second execution after durable final reconciliation", async () => {
    nhlApi.getScore.mockResolvedValue({ games: [game(1, "OFF")] });
    storage.getDailyGamesBySport.mockResolvedValue([]);
    storage.getDailyGameByGameId.mockResolvedValue({ id: "row-1", gameId: "nhl_1", status: "completed" });
    storage.getPlayersByIds.mockResolvedValue([{ id: "nhl_77" }]);
    nhlApi.getBoxscore.mockResolvedValue(finalBoxscore());
    storage.getGameStatsByGameId
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([{ statsJson: { finalReconciliation: { status: "complete", expectedPlayerCount: 1 } } }]);
