...
function parseGameTimeFromStatus(status: string, fallbackDate: Date): Date {
  ...
  const timeMatch = status.match(
    /(\d{1,2})\/(\d{1,2})\s*-\s*(\d{1,2}):(\d{2})\s*(AM|PM)\s*(?:EST|ET)/i,
  );
...
export async function syncNFLSchedule(): Promise<SyncResult> {
  ...
  try {
    ...
    console.log(
      `[NFL Schedule Sync] Fetching games for season ${season}, dates: ${dates[0]} to ${dates[dates.length - 1]}`,
    );
    ...
    for (const apiGame of apiGames) {
      ...
      try {
        ...
        const isTimePattern =
          rawStatus.match(/\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}:\d{2}\s*(am|pm)/i) ||
        ...
        if (rawStatus === "final") {
          ...
          (!isTimePattern &&
            !["postponed", "canceled", "cancelled", "scheduled"].includes(rawStatus))
        ) {
        ...
        if (result.gamesProcessed <= 5) {
          console.log(
            `[NFL Schedule Sync] DEBUG: Game ${gameId} (${apiGame.visitor_team?.abbreviation}@${apiGame.home_team?.abbreviation}): rawStatus="${apiGame.status}" -> mapped to "${status}", homeScore=${apiGame.home_team_score}, awayScore=${apiGame.visitor_team_score}`,
          );
        }
        ...
        if (hasRealTime) {
          ...
          apiGame.status &&
          apiGame.status.match(/\d{1,2}\/\d{1,2}\s*-\s*\d{1,2}:\d{2}\s*(AM|PM)/i)
        ) {
          // Parse time from status field (e.g., "12/27 - 8:00 PM EST")
          startTime = parseGameTimeFromStatus(apiGame.status, apiDate);
          console.log(
            `[NFL Schedule Sync] Parsed time: "${apiGame.status}" -> ${startTime.toISOString()}`,
          );
        } else {
          // Fallback to date field
          startTime = apiDate;
          console.log(
            `[NFL Schedule Sync] Using API date for ${apiGame.home_team?.abbreviation} vs ${apiGame.visitor_team?.abbreviation}: ${startTime.toISOString()}`,
          );
        }
        ...
        if (existingGame) {
          ...
          console.log(
            `[NFL Schedule Sync] Updating ${apiGame.visitor_team?.abbreviation} @ ${apiGame.home_team?.abbreviation}: startTime=${startTime.toISOString()}`,
          );
...
export async function syncNFLWeek(week: number): Promise<SyncResult> {
  ...
  try {
    ...
    result.success = result.errors.length === 0;
    console.log(
      `[NFL Schedule Sync] Week ${week}: ${result.gamesAdded} added, ${result.gamesUpdated} updated`,
    );
  } catch (error: any) {
    result.errors.push(`Fatal error: ${error.message}`);
  }
