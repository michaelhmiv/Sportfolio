...
export async function updateContestStatuses(
  ...
  try {
    ...
    for (const contest of openContests) {
      try {
        ...
        if (contest.startsAt && new Date(contest.startsAt) <= now) {
          console.log(
            `[update_contest_statuses] Transitioning contest ${contest.id} (${contest.name}) from "open" to "live"`,
          );
          ...
          console.log(
            `[update_contest_statuses] Contest ${contest.id} (${contest.name}) not ready to go live yet`,
          );
          console.log(`[update_contest_statuses]   - startsAt: ${contest.startsAt}`);
          console.log(`[update_contest_statuses]   - Current time: ${now.toISOString()}`);
        }
        ...
        console.error(
          `[update_contest_statuses] Failed to update contest ${contest.id}:`,
          error.message,
        );
        errorCount++;
    ...
    if (contestsProcessed > 0) {
      console.log(
        `[update_contest_statuses] Updated ${contestsProcessed} contests to "live" status`,
      );
    } else {
