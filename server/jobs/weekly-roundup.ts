...
async function gatherWeeklyStats(startDate: Date, endDate: Date): Promise<WeeklyStats> {
  ...
  const totalVolume = recentTrades.reduce(
    (sum: number, t: Trade) => sum + parseFloat(t.quantity) * parseFloat(t.price),
    0,
  );
...
function generateBlogContent(
  ...
  // Generate excerpt
  const excerpt =
    `This week saw ${stats.totalTrades} trades with $${stats.totalVolume.toFixed(0)} in total volume. ` +
    `${
