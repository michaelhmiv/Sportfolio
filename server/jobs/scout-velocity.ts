...
export async function calculateScoutVelocity(): Promise<ScoutVelocityData[]> {
  ...
  info(`[scout_velocity] Calculated velocity for ${results.length} players, ${trendingPlayers.length} trending`);
  
  return results;
}

// Job runner for scheduler
export async function runScoutVelocityJob(): Promise<{ recordsProcessed: number; errorCount: number }> {
  try {
