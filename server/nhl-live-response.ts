...
export function buildNhlLiveResponse(game: Game, stats: Stat[], players: Map<string, Player>) {
  const mapped = stats.map((stat) => {
    ...
    return {
      ...
      shotsOnGoal: number(values.shotsOnGoal),
      hits: number(values.hits),
      blockedShots: number(values.blockedShots),
      saves: nullableNumber(values.saves),
      goalsAgainst: nullableNumber(values.goalsAgainst),
      timeOnIce: values.timeOnIce || null,
      decision: values.decision || null,
      fantasyPoints: number(stat.fantasyPoints),
    };
  });
  ...
  const state =
    (stats.find((stat) => (stat.statsJson as any)?.liveState)?.statsJson as any)?.liveState || {};
  return {
    gameId: game.gameId,
    sport: "NHL",
    status: game.status,
    period: nullableNumber(state.period),
    ...
    homePlayers,
    awayPlayers,
    homeTopPerformers: top(homePlayers),
    awayTopPerformers: top(awayPlayers),
    message: mapped.length ? undefined : "No box score available yet",
  };
}
