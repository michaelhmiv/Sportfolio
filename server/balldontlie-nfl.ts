...
export async function fetchActivePlayers(): Promise<NFLPlayer[]> {
  ...
  do {
    ...
    try {
      ...
      console.log(
        `[NFL API] Fetched page ${pageCount}: ${players.length} players (total: ${allPlayers.length})`,
      );
    } catch (error: any) {
