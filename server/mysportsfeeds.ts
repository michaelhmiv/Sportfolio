...
export async function fetchActivePlayers(): Promise<MySportsFeedsPlayer[]> {
  try {
    ...
    console.log(
      `[MySportsFeeds] Success! Fetched ${response.data.playerStatsTotals?.length || 0} players`,
    );
...
export async function fetchDailyPlayerGameLogs(date: Date): Promise<any[]> {
  try {
    ...
    console.error(
      `[MySportsFeeds] Error fetching daily gamelogs for ${date.toISOString()}:`,
      error.message,
    );
    throw error;
  }
...
export async function fetchPlayerGameLogs(playerId: string, limit: number = 100): Promise<any[]> {
  ...
  console.warn(
    "[MySportsFeeds] fetchPlayerGameLogs called - consider using fetchDailyPlayerGameLogs for backfill",
  );
