...
describe("runtime-adapter internal mcp merge", () => {
  it("loads the runtime tool catalog when no explicit catalog is supplied", async () => {
    ...
    expect(mocks.getAgentRuntimeToolCatalog).toHaveBeenCalledWith({
      includeInternalMlbMcp: true,
    });
  ...
  it("excludes internal MLB tools when the profile has the built-in source disabled", async () => {
    mocks.getAgentRuntimeToolCatalog.mockResolvedValue([
      {
        toolName: "get_balance_state",
        category: "read",
    ...
    expect(mocks.getAgentRuntimeToolCatalog).toHaveBeenCalledWith({
      includeInternalMlbMcp: false,
    });
    expect(request.toolCatalog.map((entry) => entry.toolName)).toEqual(["get_balance_state"]);
  });
});
