...
export const LEGACY_SCOUT_AGENT_SYSTEM_PROMPT_V2 =
...
export const LEGACY_SCOUT_AGENT_USER_PROMPT_TEMPLATE_V2 =
...
export const LEGACY_PORTFOLIO_AGENT_SYSTEM_PROMPT_V1 = [
  ...
  "- DAILY BOOSTS: 4 slots per day per sport with tiers 5x, 4x, 3x, 2x. Each slot takes exactly 1 share (burned at game lock). Payout = shareMultiplier x fantasyPoints x (slotTier + communityBoostCount). Higher-multiplier stacked shares are more valuable in boost slots.",
...
export const LEGACY_PORTFOLIO_AGENT_USER_PROMPT_TEMPLATE_V1 =
...
export const DEFAULT_PORTFOLIO_AGENT_SYSTEM_PROMPT = [
  "You are Hermes, Sportfolio's product operator.",
  "Stay inside Sportfolio gameplay and user experience: portfolio state, player markets, liquidity, boosts, scouts, watchlists, lineups, schedules, stats, and guardrailed strategies.",
  "Use Sportfolio-native tools as the source of truth for account and gameplay state.",
  "Treat built-in or user-connected MCP sources as optional enrichment after native Sportfolio context, not as canonical state.",
  "Keep the focus on the next useful Sportfolio decision instead of acting like a general personal assistant.",
...
export const DEFAULT_PORTFOLIO_AGENT_USER_PROMPT_TEMPLATE = [
  "Act like my Sportfolio portfolio operator.",
  "Keep me focused on the highest-signal Sportfolio decision, use lineups, schedules, stats, and news only when they change what I should do, and turn direct requests into the safest staged move the current Hermes tools support.",
].join(" ");

export const DEFAULT_PORTFOLIO_AGENT_THREAD_DOMAIN = "sportfolio";

function normalizeLegacyPromptValue(value: string) {
  return value.trim().replace(/\s+/g, " ");
}

export function isLegacyScoutAgentSystemPrompt(value: string) {
  const normalized = normalizeLegacyPromptValue(value);
  return (
    normalized === normalizeLegacyPromptValue(LEGACY_SCOUT_AGENT_SYSTEM_PROMPT) ||
    normalized === normalizeLegacyPromptValue(LEGACY_SCOUT_AGENT_SYSTEM_PROMPT_V2)
  );
}

export function isLegacyScoutAgentUserPromptTemplate(value: string) {
  const normalized = normalizeLegacyPromptValue(value);
  return (
    normalized === normalizeLegacyPromptValue(LEGACY_SCOUT_AGENT_USER_PROMPT_TEMPLATE) ||
    normalized === normalizeLegacyPromptValue(LEGACY_SCOUT_AGENT_USER_PROMPT_TEMPLATE_V2)
  );
}
