...
function computeNextRunAt(jobType: AgentScheduleJobType, from = new Date()): Date {
  const config = DEFAULT_SCHEDULE_CONFIG[jobType];
  return new Date(from.getTime() + config.intervalMs);
}
...
export async function ensureDefaultUserAgentSchedules(userId: string): Promise<void> {
  ...
  await db
    ...
    .values({
      ...
      nextRunAt: computeNextRunAt("daily_setup_review", now),
    })
    .onConflictDoNothing({
...
export async function upsertUserAgentSchedule(input: {
  ...
  const now = new Date();
  const scheduleCron =
    input.scheduleCron?.trim() || DEFAULT_SCHEDULE_CONFIG[input.jobType].scheduleCron;
  const channelTargets = normalizeChannelTargets(input.channelTargets);
  ...
  const [row] = await db
    ...
    .values({
      ...
      channelTargets,
      policy: input.policy || {},
      nextRunAt: computeNextRunAt(input.jobType, now),
      updatedAt: now,
    })
    .onConflictDoUpdate({
      target: [userAgentSchedules.userId, userAgentSchedules.jobType],
  ...
  for (const row of rows) {
    ...
    try {
      ...
      await db
        ...
        .set({
          lastRunAt: now,
          nextRunAt: computeNextRunAt(schedule.jobType, now),
          updatedAt: now,
        })
