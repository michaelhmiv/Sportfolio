...
async function claimVestingShares(
  ...
  // Get current state
  const [vestingRecord] = await db.select().from(vesting).where(eq(vesting.id, vestingId));
  ...
  if (splits.length > 0) {
    ...
    const remainder =
      totalSharesClaimed - distributions.reduce((sum: number, d) => sum + d.shares, 0);
...
async function selectPlayersForVesting(
  ...
  const eligiblePlayers = await db
    ...
    .from(players)
    .where(and(eq(players.isActive, true), eq(players.isEligibleForVesting, true)));
...
async function initializeBotVesting(userId: string): Promise<void> {
  ...
  if (selectedPlayerId) {
    console.log(
      `[VestingBot] Initialized vesting for user ${userId} with player ${selectedPlayerId}`,
    );
  }
}
...
export async function executeVestingStrategy(
  ...
  if (accrued.newTotal >= claimThresholdShares) {
    console.log(
      `[VestingBot] ${profile.botName} claiming ${accrued.newTotal} shares (threshold: ${claimThresholdShares})`,
    );

    const result = await claimVestingShares(vestingState.id, currentPlayerId, config.userId);
    ...
    if (result.sharesClaimed > 0) {
      console.log(
        `[VestingBot] ${profile.botName} successfully claimed ${result.sharesClaimed} vesting shares`,
      );
    }
  }
  ...
  if (Math.random() < 0.1) {
    const candidates = await getVestingCandidates(20);
    const selectedPlayers = await selectPlayersForVesting(candidates, config.maxPlayersToVest);
    ...
    if (selectedPlayers.length > 0) {
      ...
      console.log(
        `[VestingBot] ${profile.botName} updated vesting splits to ${selectedPlayers.length} players`,
      );
    }
  }
}
