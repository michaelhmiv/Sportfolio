...
const mocks = vi.hoisted(() => ({
  ...
  storageGetActiveRewardedScoutBoostForUser: vi.fn(),
  storageGetUser: vi.fn(),
  storageUpdateUserPremiumStatus: vi.fn(),
}));
...
vi.mock("../storage", () => ({
  storage: {
    ...
    getActiveRewardedScoutBoostForUser: mocks.storageGetActiveRewardedScoutBoostForUser,
    getUser: mocks.storageGetUser,
    updateUserPremiumStatus: mocks.storageUpdateUserPremiumStatus,
  },
}));
...
describe("deterministic-engine policy utilities", () => {
  beforeEach(() => {
    ...
    mocks.storageGetActiveRewardedScoutBoostForUser.mockReset();
    mocks.storageGetUser.mockReset();
    mocks.storageUpdateUserPremiumStatus.mockReset();

    const selectQueue: unknown[][] = [[{ balance: "10000" }], []];

    mocks.dbSelect.mockImplementation(() => createSelectChain(selectQueue.shift() || []));
    mocks.storageGetUser.mockResolvedValue({
      id: "bot_user_1",
      isPremium: false,
      premiumExpiresAt: null,
    });
    mocks.storageGetActiveRewardedScoutBoostForUser.mockResolvedValue(null);
  ...
  it("preserves all-day active hour profile overrides", async () => {
      ...
      createSelectChain([
        {
          bot_profiles: {
            ...
            maxPlayerExposurePercent: "12.00",
            activeHoursStart: 0,
            activeHoursEnd: 24,
            minOrderSize: 10,
            maxOrderSize: 150,
    ...
    const [profile] = await __deterministicEngineTestHooks.loadActiveBots();

    expect(profile.activeHoursStart).toBe(0);
    expect(profile.activeHoursEnd).toBe(24);
  });

  it("loads boosted scout capacity from user entitlements", async () => {
    mocks.storageGetUser.mockResolvedValueOnce({
      id: "bot_user_1",
      isPremium: true,
      premiumExpiresAt: null,
    });

    const state = await __deterministicEngineTestHooks.loadBotState({
      userId: "bot_user_1",
      profileId: "bot_profile_1",
      botName: "Premium Bot",
      role: "market_maker",
      ...
      scoutTargetCount: 8,
      scoutRotationHours: 168,
      allowedActions: ["scout_assign"],
      actionWeights: {},
    } as any);

    expect(state?.maxScouts).toBe(10);
  });
