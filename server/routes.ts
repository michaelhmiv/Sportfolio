...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Checkout finalization endpoint - deterministic receipt reconciliation for authenticated users
  app.post("/api/checkout/finalize", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const receiptId = (req.body?.receiptId || req.body?.receipt_id || req.body?.paymentId || req.body?.payment_id || "").toString().trim();

      if (!receiptId) {
        return res.status(400).json({ error: "receiptId is required" });
      }

      const existing = await storage.getWhopPaymentByPaymentId(receiptId);
      if (existing?.creditedAt) {
        if (existing.userId !== userId) {
          return res.status(403).json({ error: "Payment is not owned by this user" });
        }
        return res.json({ success: true, status: "credited", alreadyCredited: true, assetType: existing.quantity > 0 && existing.amountCents < 500 ? "community" : "premium", quantity: existing.quantity });
      }
      ...
      // Best-effort sync to ingest recent payment from Whop
      await syncWhopPaymentsForUser(userId, user.email);

      const afterSync = await storage.getWhopPaymentByPaymentId(receiptId);
      if (!afterSync) {
        return res.status(202).json({ success: false, status: "pending", message: "Payment not available yet. Please retry in a few seconds." });
      }

      if (afterSync.creditedAt) {
        if (afterSync.userId !== userId) {
          return res.status(403).json({ error: "Payment is not owned by this user" });
        }
        return res.json({ success: true, status: "credited", alreadyCredited: true, assetType: afterSync.amountCents < 500 ? "community" : "premium", quantity: afterSync.quantity });
      }

      if (afterSync.whopStatus !== "paid") {
        return res.status(409).json({ success: false, status: "not_paid", message: `Payment status is ${afterSync.whopStatus}` });
      }

      // Deterministic ownership check: require user email match for manual finalization
      if ((afterSync.email || "").toLowerCase() !== user.email.toLowerCase()) {
        return res.status(403).json({ error: "Payment email does not match account email" });
      }

      const assetType = afterSync.amountCents < 500 ? "community" : "premium";
      const avgCost = assetType === "community" ? "1.0000" : "5.0000";
      const quantity = Math.max(1, afterSync.quantity || Math.floor(afterSync.amountCents / (assetType === "community" ? 100 : 500)));

      const creditResult = await creditPaymentAndHoldingAtomic(receiptId, userId, assetType, quantity, avgCost);
      if (!creditResult) {
        const current = await storage.getWhopPaymentByPaymentId(receiptId);
        if (current?.creditedAt && current.userId === userId) {
          return res.json({ success: true, status: "credited", alreadyCredited: true, assetType, quantity: current.quantity || quantity });
        }
        return res.status(409).json({ success: false, status: "conflict", message: "Payment already credited to a different account" });
      }
      ...
      return res.json({
        ...
        assetType,
        quantity,
        previousQuantity: creditResult.previousQuantity,
        newQuantity: creditResult.newQuantity,
      });
    } catch (error: any) {
