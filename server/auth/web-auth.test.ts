...
describe("passwordless web continuation policy", () => {
  it("exposes passwordless capabilities only when their feature flags are active", () => {
    expect(getPublicAuthCapabilities({ AUTH_MAGIC_LINK_ENABLED: "false" }).passwordlessWeb).toBe(
      false,
    );
    expect(getPublicAuthCapabilities({ AUTH_MAGIC_LINK_ENABLED: "true" }).passwordlessWeb).toBe(
      true,
    );
    expect(
      getPublicAuthCapabilities({
