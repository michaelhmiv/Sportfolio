...
async function main() {
  ...
  for (const slugPlayer of slugPlayers) {
    const apiPlayer = apiPlayers.find(
      (p) =>
    ...
    if (apiPlayer) {
      console.log(
        `✅ ${slugPlayer.firstName} ${slugPlayer.lastName}: ${slugPlayer.slug} → ${apiPlayer.id}`,
      );
      mappings.push({
      ...
      console.log(
        `❌ ${slugPlayer.firstName} ${slugPlayer.lastName}: ${slugPlayer.slug} → NOT FOUND IN API`,
      );
      mappings.push({
        slug: slugPlayer.slug,
        numericId: null,
  ...
  for (const mapping of mappings) {
    if (mapping.numericId) {
      ...
      console.log(
        `UPDATE holdings SET asset_id = '${mapping.numericId}' WHERE asset_id = '${mapping.slug}';`,
      );
      console.log(
        `UPDATE orders SET player_id = '${mapping.numericId}' WHERE player_id = '${mapping.slug}';`,
      );
      console.log(
        `UPDATE player_game_stats SET player_id = '${mapping.numericId}' WHERE player_id = '${mapping.slug}';`,
      );
