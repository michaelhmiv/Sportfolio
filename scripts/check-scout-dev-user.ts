...
async function debug() {
  ...
  assignments.forEach((a) => {
    console.log(
      `   - ${a.player.firstName} ${a.player.lastName} (${a.player.team}): ${a.scoutCount} scouts`,
    );
  });
  ...
  history.forEach((h) => {
    console.log(
      `   - Player ${h.playerId}: ${h.scoutCount} scouts, started ${h.startedAt}, ended: ${h.endedAt || "OPEN"}`,
    );
  });
  ...
  logs.forEach((l) => {
    console.log(
      `   - ${l.scheduledFor}: ${l.status} | ${l.recordsProcessed} records, ${l.errorCount} errors`,
    );
