...
async function migrate() {
  try {
    ...
    if (playerCols.rows.some((c) => c.column_name === "is_eligible_for_mining")) {
      ...
      await pool.query(
        "ALTER TABLE players RENAME COLUMN is_eligible_for_mining TO is_eligible_for_vesting",
      );
    }

    // Market Snapshots: shares_mined -> shares_vested
    const marketCols = await pool.query(
      "SELECT column_name FROM information_schema.columns WHERE table_name = 'market_snapshots'",
    );
    ...
    const botCols = await pool.query(
      "SELECT column_name FROM information_schema.columns WHERE table_name = 'bot_profiles'",
    );
    if (botCols.rows.some((c) => c.column_name === "mining_claim_threshold")) {
      ...
      await pool.query(
        "ALTER TABLE bot_profiles RENAME COLUMN mining_claim_threshold TO vesting_claim_threshold",
      );
    }
    if (botCols.rows.some((c) => c.column_name === "max_players_to_mine")) {
      ...
      await pool.query(
        "ALTER TABLE bot_profiles RENAME COLUMN max_players_to_mine TO max_players_to_vest",
      );
    }
