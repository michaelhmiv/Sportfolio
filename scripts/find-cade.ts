...
async function debug() {
  ...
  const cunninghamPlayers = await db
    .select()
    .from(players)
  ...
  const [holdingPlayer] = await db.select().from(players).where(eq(players.id, "nba_31030"));

  if (holdingPlayer) {
    console.log(
      `Found: ${holdingPlayer.firstName} ${holdingPlayer.lastName} (${holdingPlayer.team}) - ${holdingPlayer.id}`,
    );
  } else {
  ...
  const cadePlayers = await db.select().from(players).where(like(players.firstName, "Cade%"));
