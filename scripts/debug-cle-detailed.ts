...
async function run() {
    ...
    games.forEach(g => {
        ...
        console.log(`  Local: ${g.startTime.toLocaleString("en-US", { timeZone: "America/New_York" })}`);
    ...
    try {
        ...
        const cleGame = apiGames.find((g: any) => g.schedule.homeTeam.abbreviation === 'CLE' || g.schedule.awayTeam.abbreviation === 'CLE');
        if (cleGame) {
