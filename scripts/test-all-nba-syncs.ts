...
async function runSyncTests() {
  ...
  const results: Record<
    string,
    { success: boolean; recordsProcessed: number; errors: number; duration: number }
  > = {};
  ...
  try {
    ...
    console.log(
      `✓ roster_sync: ${result.recordsProcessed} players, ${result.errorCount} errors, ${results.roster_sync.duration}ms`,
    );
  } catch (error: any) {
  ...
  try {
    ...
    console.log(
      `✓ schedule_sync: ${result.recordsProcessed} games, ${result.errorCount} errors, ${results.schedule_sync.duration}ms`,
    );
  } catch (error: any) {
  ...
  try {
    ...
    console.log(
      `✓ stats_sync: ${result.recordsProcessed} stats, ${result.errorCount} errors, ${results.stats_sync.duration}ms`,
    );
  } catch (error: any) {
  ...
  try {
    ...
    console.log(
      `✓ sync_player_game_logs: ${result.recordsProcessed} stats, ${result.errorCount} errors, ${results.sync_player_game_logs.duration}ms`,
    );
  } catch (error: any) {
  ...
  for (const [job, result] of Object.entries(results)) {
    ...
    console.log(
      `${status} | ${job}: ${result.recordsProcessed} records, ${result.errors} errors, ${result.duration}ms`,
    );
  ...
  console.log(
    allPassed
      ? "ALL TESTS PASSED - BallDontLie integration verified!"
