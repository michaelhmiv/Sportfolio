...
async function debug() {
    ...
    const [cade] = await db.select().from(players)
        .where(and(
            eq(players.lastName, 'Cunningham'),
            eq(players.team, 'DET')
        ));
    ...
    for (const h of userHoldings) {
        console.log(`  ${h.player.firstName} ${h.player.lastName} (${h.player.team}): ${h.holding.quantity} shares, PL: ${h.holding.powerLevel || '0'}`);
    }

    // Check Cade's holding
    console.log("\n--- Cade Cunningham Holding ---");
    const [cadeHolding] = await db.select().from(holdings)
        .where(and(
            eq(holdings.userId, user.id),
            eq(holdings.assetId, cade.id)
        ));
    ...
    const locks = await db.select().from(holdingsLocks)
        .where(and(
            eq(holdingsLocks.userId, user.id),
            eq(holdingsLocks.assetId, cade.id)
        ));

    if (locks.length > 0) {
        console.log(`Found ${locks.length} lock(s):`);
        for (const lock of locks) {
            console.log(`  - Type: ${lock.lockType}, Qty: ${lock.lockedQuantity}, Ref: ${lock.lockReferenceId}`);
        }
    } else {
    ...
    const todaysGames = await db.select().from(dailyGames)
        .where(and(
            ...
            or(
                eq(dailyGames.homeTeam, cade.team),
                eq(dailyGames.awayTeam, cade.team)
            ),
    ...
    const allNbaGames = await db.select().from(dailyGames)
        .where(and(
            eq(dailyGames.sport, "NBA"),
            gte(dailyGames.startTime, startOfDay),
            lt(dailyGames.startTime, endOfDay)
    ...
    if (cadeEligible) {
        ...
        for (const e of eligible) {
            console.log(`  - ${e.player.firstName} ${e.player.lastName} (${e.player.team}): ${e.availableShares} shares, PL: ${e.powerLevel}`);
        }
    }
