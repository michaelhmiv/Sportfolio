...
async function cleanupDuplicateGames() {
    ...
    console.log(`Found ${legacyIds.length} legacy MySportsFeeds records (gameId starting with 18447)`);
    ...
    for (const legacy of legacyIds) {
        ...
        const legacyRecord = await db
            .select()
            .from(dailyGames)
            .where(eq(dailyGames.id, legacy.id));
        ...
        if (possibleMatches.length > 0) {
            ...
            console.log(`Deleting legacy ${legacy.gameId} (${rec.awayTeam}@${rec.homeTeam}) - BDL equivalent exists: ${possibleMatches[0].gameId}`);
