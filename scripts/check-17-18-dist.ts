...
async function check() {
  ...
  if (historyRows.length === 0) {
    ...
    historyRows.forEach(h => {
      console.log(`  ${h.user_id.substring(0,8)}... | ${h.scout_count} scouts | Overlap: ${h.overlap_minutes || 0} min`);
    });
  }
  ...
  const dists = await db.select().from(scoutDistributions)
    .where(sql`${scoutDistributions.playerId} = 'nba_31030' AND ${scoutDistributions.hourTimestamp} = ${windowEnd}::timestamp`);
