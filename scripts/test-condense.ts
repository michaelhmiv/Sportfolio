...
async function testCondenseShares() {
    ...
    if (!eligibleHolding) {
        ...
        if (player) {
            ...
            const refreshedHoldings = await db.select().from(holdings).where(and(eq(holdings.userId, testUser.id), eq(holdings.assetType, "player")));
    ...
    try {
        ...
        if (updatedHolding) {
            console.log(`   Updated quantity: ${updatedHolding.quantity} (expected: ${originalQuantity - 5})`);
            console.log(`   Updated power level: ${updatedHolding.powerLevel} (expected: ${(originalPowerLevel + 1).toFixed(2)})`);

            if (updatedHolding.quantity === originalQuantity - 5 &&
