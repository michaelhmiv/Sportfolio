...
async function debug() {
  ...
  for (const g of games) {
    ...
    console.log(
      `  EST time: ${new Date(g.startTime).toLocaleString("en-US", { timeZone: "America/New_York" })}`,
    );
    console.log(`  \n`);
  }
  ...
  const boundaryGames = await db
    .select()
    .from(dailyGames)
    .where(eq(dailyGames.date, new Date("2026-01-20T00:00:00.000Z")));
  ...
  for (const g of boundaryGames) {
    ...
    if (startHourUTC < 5) {
      // Before 5 AM UTC = before midnight EST
      console.log(
        `  ${g.awayTeam} @ ${g.homeTeam}: startTime=${g.startTime} (${startHourUTC} UTC)`,
      );
    }
  }
}
