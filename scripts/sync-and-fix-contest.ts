...
async function fixNFLContestTimes() {
  ...
  console.log(
    `Sync result: ${syncResult.gamesProcessed} processed, ${syncResult.gamesUpdated} updated\n`,
  );
  ...
  console.log(
    `Looking for games between ${startOfDayET.toISOString()} and ${endOfDayET.toISOString()}\n`,
  );
  ...
  for (const g of games) {
    ...
    console.log(
      `  ${g.awayTeam} @ ${g.homeTeam}: ${format(startTimeET, "h:mm a", { timeZone: "America/New_York" })} ET`,
    );
  }
  ...
  console.log(
    `Start time: ${format(toZonedTime(earliestStartTime, "America/New_York"), "h:mm a", { timeZone: "America/New_York" })} ET`,
  );
  ...
  const todayNFLContest = allContests.find((c) => {
    ...
    const contestDateStr = format(toZonedTime(c.gameDate, "America/New_York"), "yyyy-MM-dd", {
  ...
  console.log(
    `Current starts_at: ${format(toZonedTime(todayNFLContest.startsAt, "America/New_York"), "h:mm a", { timeZone: "America/New_York" })} ET`,
  );
  ...
  console.log(
    `Updated to: ${format(toZonedTime(earliestStartTime, "America/New_York"), "h:mm a", { timeZone: "America/New_York" })} ET`,
  );
  console.log("\nDone!");
}
