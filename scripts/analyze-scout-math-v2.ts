...
async function analyzeDistribution() {
  ...
  dists.forEach((d) => {
    ...
    console.log(
      `  Received: ${d.shares} | UserMin: ${d.userMinutes} | GlobalMin: ${d.globalMinutes}`,
    );
  ...
  for (const [playerId, records] of Object.entries(byPlayer)) {
    ...
    records.forEach((h) => {
      ...
      if (overlapEnd > overlapStart) {
        ...
        if (
          h.userId === "dev-user-12345678" ||
          h.userId.substring(0, 8) === "83eeaf7" ||
          scoutMinutes > 50
        ) {
          console.log(
            `  User ${h.userId.substring(0, 8)}...: ${h.scoutCount} scouts × ${overlapMinutes.toFixed(1)} min = ${scoutMinutes.toFixed(1)} scout-minutes`,
          );
        }
      }
    });
    ...
    if (devUserRecord) {
      ...
      console.log(
        `  dev-user-12345678: ${devUserRecord.scoutCount} scouts × ${overlapMinutes.toFixed(1)} min = ${devMinutes.toFixed(1)} scout-minutes`,
      );
      console.log(
        `  Your expected share: ${((devMinutes / totalPlayerScoutMinutes) * 60).toFixed(2)} shares`,
      );
    }
    ...
    if (playerDist) {
      console.log(
        `  DISTRIBUTION: ${playerDist.shares} shares to user ${playerDist.userId.substring(0, 8)}...`,
      );
    }
  }
}
