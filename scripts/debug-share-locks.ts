...
async function debug() {
    ...
    for (const h of userHoldings) {
        console.log(`  ${h.player.firstName} ${h.player.lastName} (${h.player.team}): ${h.holding.quantity} shares, powerLevel: ${h.holding.powerLevel}`);
    }
    ...
    for (const h of userHoldings) {
        ...
        if (locks.length > 0) {
            ...
            for (const lock of locks) {
                console.log(`      - ${lock.lockType}: ${lock.lockedQuantity} (ref: ${lock.lockReferenceId})`);
            }
        }
    }
    ...
    for (const h of userHoldings.slice(0, 3)) {
        ...
        console.log(`  ${h.player.firstName} ${h.player.lastName}: getTotalLockedQuantity=${lockedQty}`);
    }
