...
async function debug() {
    ...
    todaysGames.forEach((game, i) => {
        ...
        console.log(`  ${i + 1}. ${game.awayTeam} @ ${game.homeTeam} (${new Date(game.startTime).toLocaleTimeString()})`);
    });
    ...
    for (const h of userHoldings) {
        ...
        const status = hasGame && hasShares ? "✅ ELIGIBLE" :
            !hasGame ? "❌ No game" :
                "❌ No shares";

        if (hasGame) matchCount++;

        console.log(`  ${h.player.firstName} ${h.player.lastName} (${team}) | Qty: ${h.holding.quantity} PL: ${h.holding.powerLevel || '0.00'} | ${status}`);
    }
    ...
    if (eligible.length > 0) {
        eligible.forEach((ep) => {
            console.log(`  ✅ ${ep.player.firstName} ${ep.player.lastName} | Shares: ${ep.availableShares} | PL: ${ep.powerLevel}`);
        });
    }
