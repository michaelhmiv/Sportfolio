...
async function debug() {
    ...
    const [brunson] = await db.select().from(players)
        .where(and(
            eq(players.lastName, 'Brunson'),
            eq(players.team, 'NYK')
        ));
    ...
    console.log(`Player: ${brunson.firstName} ${brunson.lastName} (${brunson.team}) - ID: ${brunson.id}`);
    ...
    if (brunsonResult) {
        ...
        for (const r of result) {
            console.log(`  - ${r.player.firstName} ${r.player.lastName} (${r.player.team}): ${r.availableShares} shares`);
        }
    }
