...
package["scripts"]["auth:foundation:test"] = "vitest run server/auth/principal.test.ts server/auth/better-auth.test.ts server/supabaseAuth.test.ts"
...
export function mountBetterAuthHandler(app: Express, config: AuthRuntimeConfig = getAuthRuntimeConfig()): boolean {
  ...
  app.all(`${BETTER_AUTH_BASE_PATH}/sign-up/email`, (_req, res) => res.status(404).json({ error: "Password registration is not available" }));
  app.all(`${BETTER_AUTH_BASE_PATH}/sign-in/email`, (_req, res) => res.status(404).json({ error: "Password login is not available" }));
  app.all(`${BETTER_AUTH_BASE_PATH}/*`, toNodeHandler(auth));
  logger.info({ provider: config.AUTH_PROVIDER, basePath: BETTER_AUTH_BASE_PATH, trustedOriginCount: trustedOrigins(config).length }, "Mounted Better Auth handler");
  return true;
}
''')
...
describe("provider-neutral authentication principal", () => {
  it("keeps canonical Sportfolio user id as the request subject", () => {
    expect(toLegacyRequestUser({ userId: "canonical-user", provider: "better-auth", providerSubject: "auth-user" }).claims.sub).toBe("canonical-user");
  });
  it("attaches the principal and temporary legacy claim shape", () => {
    const req = {} as Request;
    attachAuthPrincipal(req, { userId: "canonical-user", provider: "supabase", providerSubject: "supabase-subject" });
...
import { BETTER_AUTH_BASE_PATH, BETTER_AUTH_SESSION_EXPIRES_IN_SECONDS, BETTER_AUTH_SESSION_UPDATE_AGE_SECONDS, createBetterAuthServer, mountBetterAuthHandler } from "./better-auth";
...
describe("Better Auth server foundation", () => {
  ...
  it("keeps password registration unavailable", async () => {
    const auth = createBetterAuthServer(testConfig(), async () => undefined);
    const response = await auth.handler(new Request(`https://auth.sportfolio.market${BETTER_AUTH_BASE_PATH}/sign-up/email`, {
      method: "POST",
...
replacement = '''    const existingUser = existingUserById || existingUserByEmail;
    ...
    const knownProviderSubjects = new Set([
      existingUser?.authProviderSubject,
      ...(existingUser?.authProviderSubjects ?? []),
    ...
    // Only generate a new username'''
if "providerSubjectAlreadyLinked" not in supabase:
    supabase = supabase.replace(existing_anchor, replacement, 1)
...
if start != -1 and end != -1:
    replacement_builder = '''function buildSupabasePrincipal(
...
'''
    supabase = supabase[:start] + replacement_builder + supabase[end + 2:]
...
supabase = supabase.replace('''(req as any).user = {
        claims: {
          sub: mockUserId,
