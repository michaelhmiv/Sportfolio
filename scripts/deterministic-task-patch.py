...
'''
write("server/auth/config.ts", config)

config_test = r'''import { describe, expect, it } from "vitest";
...
'''
write("server/auth/config.test.ts", config_test)

runbook = r'''# Sportfolio authentication environments
...
'''
write("docs/auth/railway-environments.md", runbook)

program = r'''# Passwordless authentication migration program
...
'''
write("docs/auth/migration-program.md", program)

index_path = ROOT / "server/index.ts"
index = index_path.read_text(encoding="utf-8")
import_line = 'import { getAuthDiagnostics, getAuthRuntimeConfig } from "./auth/config";'
if import_line not in index:
    anchor = 'import { isWriteMaintenanceMode, maintenanceWriteGuard } from "./maintenance-mode";'
    if anchor not in index:
        raise SystemExit("Could not find server index import anchor")
    index = index.replace(anchor, f"{anchor}\n{import_line}", 1)

runtime_anchor = "const hermesSidecarMode = isHermesSidecarMode();"
runtime_block = '''const hermesSidecarMode = isHermesSidecarMode();
...
if "Validated authentication runtime configuration" not in index:
    if runtime_anchor not in index:
        raise SystemExit("Could not find server index runtime anchor")
    index = index.replace(runtime_anchor, runtime_block, 1)
index_path.write_text(index, encoding="utf-8")

package_path = ROOT / "package.json"
package = json.loads(package_path.read_text(encoding="utf-8"))
package["scripts"]["auth:config:test"] = "vitest run server/auth/config.test.ts"
package_path.write_text(json.dumps(package, indent=2) + "\n", encoding="utf-8")

wrong_task = ROOT / "scripts/tasks/auth-mainline-foundation-370.mjs"
if wrong_task.exists():
    wrong_task.unlink()
    try:
        wrong_task.parent.rmdir()
    except OSError:
        pass
