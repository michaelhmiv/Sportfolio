...
async function analyzeDistribution() {
  ...
  const dist = await db.select().from(scoutDistributions)
    .where(eq(scoutDistributions.hourTimestamp, targetHour))
    .orderBy(scoutDistributions.playerId);

  console.log('\nAll distributions for 17:00 hour:');
  dist.forEach(d => {
    console.log(`  Player ${d.playerId}: ${d.sharesEarned} shares | User min: ${d.userScoutMinutes} | Global min: ${d.globalScoutMinutes}`);
  });
  ...
  for (const [playerId, dists] of Object.entries(playerGroups)) {
    ...
    console.log(`  Expected total (should be 60): ${(60 * totalUserMinutes / totalGlobalMinutes).toFixed(2)}`);
  }
  ...
  history.forEach(h => {
    console.log(`  User: ${h.userId} | Player: ${h.playerId} | Count: ${h.scoutCount} | Started: ${h.startedAt} | Ended: ${h.endedAt || 'OPEN'}`);
  });
  ...
  for (const [playerId, entries] of Object.entries(playerHistory)) {
    ...
    entries.forEach(e => {
      ...
      console.log(`  User ${e.userId}: ${e.count} scouts × ${e.minutes.toFixed(1)} min = ${scoutMinutes.toFixed(1)} scout-minutes`);
    });
