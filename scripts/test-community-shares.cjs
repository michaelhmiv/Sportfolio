...
async function runTests() {
  ...
  test('Should find pending community session within 2 hours', async () => {
    const pendingSessions = [
      { id: 'session-1', userId: 'user-1', planId: 'plan_Community', quantity: 1, createdAt: new Date() },
      { id: 'session-2', userId: 'user-2', planId: 'plan_Community', quantity: 2, createdAt: new Date(Date.now() - 3600000) }, // 1 hour ago
      { id: 'session-3', userId: 'user-3', planId: 'plan_Community', quantity: 3, createdAt: new Date(Date.now() - 7200000) }, // 2 hours ago - should be included
    ];
    ...
    let matchingSession = pendingSessions.find(s =>
      s.planId === planId &&
  ...
  test('Should exclude sessions older than 2 hours', async () => {
    const pendingSessions = [
      { id: 'session-old', userId: 'user-old', planId: 'plan_Community', quantity: 1, createdAt: new Date(Date.now() - 7200001) }, // 2 hours + 1ms ago
    ];

    const twoHoursAgo = new Date(Date.now() - 2 * 60 * 60 * 1000);

    const matchingSession = pendingSessions.find(s =>
      s.planId === 'plan_Community' &&
      new Date(s.createdAt) > twoHoursAgo
    );
  ...
  test('Should fallback to premium sessions when no community sessions exist', async () => {
    ...
    const premiumSessions = [
      { id: 'prem-session', userId: 'user-1', planId: 'plan_Premium', quantity: 1, createdAt: new Date() },
    ];
  ...
  test('Sync should detect community by amount < $3', async () => {
    const totalDollars = 1.50; // Community purchase
  ...
  test('Sync should detect premium by amount >= $3', async () => {
    const totalDollars = 5.00; // Premium purchase
