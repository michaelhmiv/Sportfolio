...
async function analyzeMarket() {
  ...
  console.log(
    "PLAYERS:",
    stats.rows[0].total_players,
    "total |",
    stats.rows[0].priced_players,
    "with prices (" +
  ...
  const orders = await pool.query(
    `SELECT side, COUNT(*) as c FROM orders WHERE status = 'open' GROUP BY side`,
  );
  ...
  console.log(
    "PRICE RANGE: $" +
      parseFloat(ps.min_price).toFixed(2) +
      " - $" +
      parseFloat(ps.max_price).toFixed(2) +
      " | Avg: $" +
      parseFloat(ps.avg_price).toFixed(2) +
      " | StdDev: $" +
      parseFloat(ps.stddev || 0).toFixed(2),
  );
  ...
  console.log(
    "BOT ACTIVITY: Orders:",
    botStats.rows[0].bot_orders,
    "| Trades involved:",
    botStats.rows[0].bot_trades,
  );
  ...
  console.log(
    "HOLDINGS:",
    holdings.rows[0].records,
    "records |",
    holdings.rows[0].total_shares,
    "shares |",
    holdings.rows[0].users,
    "users",
  );
  ...
  console.log(
    "VOLUME (24h): Total:",
    volumeStats.rows[0].total_volume,
    "| Avg per player:",
  ...
  topMcap.rows.forEach((p: any, i: number) => {
    console.log(
      `${i + 1}. ${p.first_name} ${p.last_name}: $${p.last_trade_price} | Vol: ${p.volume_24h} | MCap: $${parseFloat(p.market_cap).toFixed(0)} | Shares: ${p.total_shares}`,
    );
  });
  ...
  topVol.rows.forEach((p: any, i: number) => {
    console.log(
      `${i + 1}. ${p.first_name} ${p.last_name}: ${p.volume_24h} shares at $${p.last_trade_price}`,
    );
  });
