...
async function debugSentiment() {
  ...
  try {
    ...
    topSentimentScanner.slice(0, 5).forEach((s) => {
      console.log(
        `${s.player.firstName} ${s.player.lastName}: ${Math.round(s.metrics.sentiment.buyPressure)}% (Vol: ${s.metrics.sentiment.totalVolume24h})`,
      );
    });
    ...
    const stats = await db
      ...
      .from(orders)
      .where(and(inArray(orders.playerId, playerIds), gte(orders.createdAt, twentyFourHoursAgo)))
    ...
    paginatedPlayers.forEach((p) => {
      ...
      const pressure =
        s && s.totalVol > 0 ? `${Math.round((s.buyVol / s.totalVol) * 100)}%` : "N/A (50%)";
