...
async function debug() {
  ...
  for (const h of nbaHoldings) {
    ...
    if (locks.length > 0) {
      locks.forEach((l) =>
        console.log(`    - ${l.lockType}: ${l.lockedQuantity} (ref: ${l.lockReferenceId})`),
      );
    }
    ...
    if (activeBoosts.length > 0) {
      activeBoosts.forEach((b) =>
        console.log(`    - ${b.slotTier}x slot with ${b.sharesEntered} shares`),
      );
    }
  }
  ...
  for (const e of eligible) {
    console.log(
      `  ${e.player.firstName} ${e.player.lastName}: ${e.availableShares} shares available`,
    );
  }
