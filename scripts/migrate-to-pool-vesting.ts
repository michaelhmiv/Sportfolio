...
async function migrateToPoolVesting() {
  ...
  const allVestingRecords = await db.select().from(vesting).where(gt(vesting.sharesAccumulated, 0));
  ...
  for (const vestingRecord of allVestingRecords) {
    try {
      ...
      const splits = await db.select().from(vestingSplits).where(eq(vestingSplits.userId, userId));
      ...
      if (usingSplits) {
        ...
        console.log(
          `[Migration] User ${userId}: Distributed ${sharesAccumulated} shares across ${splits.length} players`,
        );
        ...
        console.log(
          `[Migration] User ${userId}: Claimed ${sharesAccumulated} shares for player ${vestingRecord.playerId}`,
        );
      } else {
        console.log(
          `[Migration] User ${userId}: Had ${sharesAccumulated} shares but no player selected - shares lost`,
        );
      }
