...
async function debug() {
    ...
    for (const h of userHoldings) {
        console.log(`  ${h.player?.firstName || 'N/A'} ${h.player?.lastName || 'N/A'} (${h.player?.team || 'N/A'}): ${h.holding.quantity} shares, powerLevel: ${h.holding.powerLevel || '0'}`);
    }

    // Get all locks for user
    console.log("\n--- All Locks ---");
    const userLocks = await db.select().from(holdingsLocks)
        .where(eq(holdingsLocks.userId, userId));
    ...
    for (const lock of userLocks) {
        ...
        console.log(`  ${playerName} (${lock.assetType}): ${lock.lockType} - ${lock.lockedQuantity} locked (ref: ${lock.lockReferenceId})`);
    }
    ...
    for (const h of userHoldings.filter(h => h.holding.quantity > 0)) {
        ...
        console.log(`  Total: ${h.holding.quantity} | Locked: ${totalLocked} | Available: ${available}`);
    }
