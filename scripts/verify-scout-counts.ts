...
async function verifyScoutCounts() {
  ...
  for (const assignment of assignments) {
    const hasOpen = openHistory.some(
      (h) => h.userId === assignment.userId && h.playerId === assignment.playerId,
    );
    if (!hasOpen) {
  ...
  for (const a of devAssignments) {
    const [historyRec] = await db
      ...
      .from(scoutHistory)
      .where(
        sql`${scoutHistory.userId} = ${a.userId} AND ${scoutHistory.playerId} = ${a.playerId} AND ${scoutHistory.endedAt} IS NULL`,
      )
      .limit(1);
    ...
    console.log(
      `    Open history: ${historyRec ? historyRec.scoutCount + " scouts (OK)" : "MISSING!"}`,
    );
  }
  ...
  const windowHistory = await db.select().from(scoutHistory).where(sql`
  ...
  for (const [playerId, records] of Object.entries(byPlayer)) {
    ...
    for (const r of records) {
      ...
      if (rOverlapEnd > rOverlapStart) {
        globalScoutMinutes +=
          r.scoutCount * ((rOverlapEnd.getTime() - rOverlapStart.getTime()) / (1000 * 60));
      }
    }
    ...
    console.log(
      `    Your: ${devRecord.scoutCount} scouts × ${overlapMin.toFixed(1)} min = ${devScoutMinutes.toFixed(1)} scout-minutes`,
    );
  ...
  console.log(
    `✓ History records match assignments: ${historyMatchCount}/${historyMatchCount + historyMismatchCount}`,
  );
  console.log(
    `${missingHistory.length === 0 ? "✓" : "⚠️"} Open history records: ${missingHistory.length === 0 ? "All present" : "Missing " + missingHistory.length}`,
  );
}
